...
const CqManagement: React.FC = () => {
  ...
  const fetchCqs = async () => {
    ...
    try {
      ...
      setError(`获取连续查询列表失败: ${err.response?.data?.message || err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    { title: 'CQ 名称', dataIndex: 'cqId', key: 'cqId' },
    {
      title: '状态',
      dataIndex: 'state',
      key: 'state',
    ...
    { title: '语句', dataIndex: 'query', key: 'query', ellipsis: true, width: 520 },
    {
      title: '操作',
      key: 'action',
      render: (_: unknown, record: CqInfo) => (
        <Popconfirm title="确定删除该连续查询吗？" onConfirm={() => handleDelete(record.cqId)}>
              ...
              locale={{
                emptyText: (
                  <Alert
                    ...
                    showIcon
                    title="暂无连续查询"
                    description="查询成功，当前集群没有连续查询。CQ 按 GROUP BY 的窗口周期重算一条聚合查询，并把结果写进 INTO 指定的目标路径；不填 RESAMPLE EVERY 时，执行间隔等于 GROUP BY 窗口大小。"
                  />
                ),
              }}
            />
      ...
      >
        {createError && (
          <Alert
            type="error"
            showIcon
            title="服务端拒绝了这条 CREATE CONTINUOUS QUERY 语句"
        ...
        >
          ...
          <Form.Item
            ...
            label="SELECT 表达式"
            rules={[
              { required: true, message: '请输入聚合表达式' },
              { pattern: FRAGMENT, message: '仅允许测点名、函数、* 与运算符，不要带引号或分号' },
            ]}
            extra="聚合函数按窗口计算，例如 max_value(temperature)。"
          >
          ...
          <Form.Item
            ...
            label="INTO 设备路径"
            rules={[
              { required: true, message: '请输入目标设备路径' },
              { pattern: FRAGMENT, message: '仅允许以 root 开头的路径字符' },
            ]}
          >
          ...
          <Form.Item
            ...
            label="INTO 目标测点"
            rules={[
              { required: true, message: '请输入目标测点名，多个用逗号分隔' },
              { pattern: METRICS, message: '测点名为字母、数字、下划线，逗号分隔' },
            ]}
            extra="INTO 必须写成 路径(测点)，数量要与 SELECT 表达式一致。"
          >
          ...
          <Form.Item
            ...
            rules={[
              ...
              { pattern: FRAGMENT, message: '仅允许路径字符与 * 通配' },
            ]}
          >
            <Input placeholder="root.sg.d1" />
          </Form.Item>
          <Form.Item
            ...
            label="GROUP BY 窗口"
            rules={[
              { required: true, message: '请输入窗口大小' },
              { pattern: DURATION, message: '时长字面量，例如 30s、1m、1h' },
            ]}
          >
          ...
          <Form.Item
            ...
            rules={[{ pattern: DURATION, message: '时长字面量，例如 30s、1m、1h' }]}
            extra="留空则按 GROUP BY 的窗口大小执行。"
          >
            <Input placeholder="30s" />
          </Form.Item>
          <Form.Item name="timeoutPolicy" label="TIMEOUT POLICY" extra="上一个窗口还没算完时怎么处理。">
            <Select allowClear placeholder="服务端默认">
