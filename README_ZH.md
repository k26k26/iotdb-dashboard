...
下面每一张图都是在真实运行的 IoTDB 2.0.11 节点上截取的 1920×911 画面，没有假数据、没有摆拍的空白页。分组与左侧菜单一致。某些页面显示的是服务端拒绝而不是表格，这是真实结果：IoTDB 2.0.11 没有对应语句，应用把拒绝原因原样报出来，而不是画一个空状态糊过去。每张图顶部显示的是作者局域网里的开发节点地址，截图数据来自那里，你在别处访问不到。
